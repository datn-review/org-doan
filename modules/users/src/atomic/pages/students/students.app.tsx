...
import { useDeleteUserStudentMutation, useLazyGetUserStudentQuery } from '@org/store';
import {
  Button,
  H2,
  IconDeleteAction,
...
function StudentsApp() {
  ...
  useEffect(() => {
    ...
    dispatch(setActiveSubGroup({ current: SiteMap.Users.Student.menu }));
    return () => {
      dispatch(clearActiveMenu());
    };
  }, []);

  const [getUser, { data, isLoading }] = useLazyGetUserStudentQuery();
  const [deleteUser] = useDeleteUserStudentMutation();
