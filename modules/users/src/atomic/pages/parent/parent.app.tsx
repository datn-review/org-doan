...
import { useDeleteParentMutation, useLazyGetParentQuery } from '@org/store';
import {
  Button,
  H2,
  IconDeleteAction,
...
function ParentApp() {
  ...
  useEffect(() => {
    ...
    dispatch(setActiveSubGroup({ current: SiteMap.Users.Parent.menu }));
    return () => {
      dispatch(clearActiveMenu());
    };
  }, []);

  const [getUser, { data, isLoading }] = useLazyGetParentQuery();
  const [deleteUser] = useDeleteParentMutation();
