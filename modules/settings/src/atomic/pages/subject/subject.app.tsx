...
import { useDeleteSubjectMutation, useLazyGetSubjectQuery } from '@org/store';
import {
  Button,
  H2,
  IconDeleteAction,
...
function SubjectApp() {
  ...
  useEffect(() => {
    ...
    dispatch(setActiveSubGroup({ current: SiteMap.Settings.Subject.menu }));
    return () => {
      dispatch(clearActiveMenu());
    };
  }, []);

  const [getUser, { data, isLoading }] = useLazyGetSubjectQuery();
  const [deleteUser] = useDeleteSubjectMutation();
