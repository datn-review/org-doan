...
import { useDeleteSkillsMutation, useLazyGetSkillsQuery } from '@org/store';
import {
  Button,
  H2,
  IconDeleteAction,
...
function SkillsApp() {
  ...
  useEffect(() => {
    ...
    dispatch(setActiveSubGroup({ current: SiteMap.Settings.Skills.menu }));
    return () => {
      dispatch(clearActiveMenu());
    };
  }, []);

  const [getUser, { data, isLoading }] = useLazyGetSkillsQuery();
  const [deleteUser] = useDeleteSkillsMutation();
