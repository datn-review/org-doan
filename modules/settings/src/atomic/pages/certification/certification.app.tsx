...
import { useDeleteCertificationMutation, useLazyGetCertificationQuery } from '@org/store';
import {
  Button,
  H2,
  IconDeleteAction,
...
function CertificationApp() {
  ...
  useEffect(() => {
    ...
    dispatch(setActiveSubGroup({ current: SiteMap.Settings.Certification.menu }));
    return () => {
      dispatch(clearActiveMenu());
    };
  }, []);

  const [getUser, { data, isLoading }] = useLazyGetCertificationQuery();
  const [deleteUser] = useDeleteCertificationMutation();
  ...
  return (
    <Space>
      ...
      <Space
        ...
        `}
      >
        <H2>{t('settings.certification')}</H2>
...
export default CertificationApp;
